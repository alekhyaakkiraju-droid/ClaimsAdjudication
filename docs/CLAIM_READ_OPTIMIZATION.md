...
- `claim/read_queryset.py` — `claim_read_queryset()` applies:
  - `select_related`: health facility, insuree, admin, icd, batch run, feedback
  - `prefetch_related`: active items, services, attachments, pending mutations

## Integration points

- `claim/schema.py` — `resolve_claims`, `resolve_claim_history` wrap the queryset before `gql_optimizer.query()`
